"""Inference pipeline for event log sequence tagging models.

This script provides end-to-end inference functionality on raw event logs:
- Loading trained model checkpoints and preprocessing artifacts
- Feature engineering from raw CSV event logs
- Sequence encoding using saved vocabularies and encoders  
- Model prediction with confidence scoring
- Optional selective prediction with confidence thresholds
- Results saved to structured CSV with predictions and confidences

The pipeline automatically reconstructs the same multimodal feature
processing used during training, ensuring consistency between training
and inference phases.

Usage:
    python src/inference.py
    
Input data should be in data/predictions.csv
Results are saved to results/predictions.csv
"""
...
def load_and_prepare_raw_dataframe(input_file: str = "data/predictions.csv") -> pd.DataFrame:
    ...
    - Sorting and NA handling consistent with training
    ...
    df = builder.load_and_prepare(input_file)
    return df

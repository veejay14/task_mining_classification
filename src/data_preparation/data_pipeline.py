"""Data preparation utilities for event log sequence modeling.

This module provides comprehensive data preparation functionality including:
- Session-based data splitting for temporal data
- Vocabulary building for event names and labels
- Encoder fitting for categorical and numeric features
- Sequence dataset classes with dynamic padding
- Optional multimodal feature encoding (text and images)
- Dataset generation and collation functions

Key functions:
    - split_by_session: Session-aware train/val/test splitting
    - build_event_vocab: Create vocabulary from event names
    - fit_encoders: Fit categorical and numeric transformers
    - build_split: Complete feature encoding pipeline
    - MultiFeatureSequenceDataset: PyTorch dataset for sequences
    - collate_batch: Dynamic padding collation function
"""
...
def split_by_session(df: pd.DataFrame,
    ...
    train_ids, val_ids = train_test_split(train_ids, test_size=val_size, random_state=seed)  # 80/10/10

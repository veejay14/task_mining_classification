# ml_baseline.py
"""
A small, human-friendly ML baseline for event logs.

What this script does (in plain English):
1) Loads your CSV of raw events.
2) Engineers simple features from `event_name` and session timing.
3) Encodes features into a model-ready matrix (one-hot for categoricals; numeric cleaned).
4) Trains a RandomForest baseline and evaluates it on validation + test splits.
5) Reports a "high-confidence" slice (>= threshold) to see where the model is most sure.

You can import the classes below into other files, or just run this script directly.
"""

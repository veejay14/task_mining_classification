"""Neural network model definitions for sequence tagging.

This module provides two main model architectures for event log sequence tagging:

1. MultiFeatureBiLSTMTagger: Bidirectional LSTM-based sequence tagger
   - Supports multimodal features (event, categorical, numeric, text, image)
   - Configurable fusion modes (concatenation or gated)
   - Dropout and regularization options

2. MultiFeatureTransformerTagger: Transformer-based sequence tagger  
   - Self-attention mechanism for sequence modeling
   - Positional encoding for sequence order
   - Same multimodal feature support as BiLSTM

Both models support:
   - Event name embeddings
   - Categorical feature embeddings  
   - Numeric feature projections
   - Optional text embeddings (via SentenceTransformer)
   - Optional image embeddings (via ResNet-50)
   - Flexible fusion strategies for multimodal inputs
"""
...
class MultiFeatureBiLSTMTagger(nn.Module):
    """Bidirectional LSTM-based sequence tagger with multimodal feature fusion.
    
    This model combines event embeddings, categorical features, numeric features,
    and optional text/image features through configurable fusion mechanisms,
    then processes sequences with bidirectional LSTM for token-wise 
    classification.
    
    Args:
        vocab_size: Size of event vocabulary
        num_classes: Number of output classes for classification
        cat_vocab_sizes: Dictionary mapping categorical feature names to vocab 
            sizes
        emb_event_dim: Event embedding dimension
        emb_cat_dim: Categorical embedding dimension  
        num_cols: List of numeric feature column names
        num_proj_dim: Numeric feature projection dimension
        txt_in_dim: Input dimension for text features (0 to disable)
        txt_proj_dim: Text feature projection dimension
        img_in_dim: Input dimension for image features (0 to disable)  
        img_proj_dim: Image feature projection dimension
        hidden_dim: LSTM hidden dimension
        num_layers: Number of LSTM layers
        pad_idx: Padding index for embeddings
        dropout_p: General dropout probability
        rnn_dropout: LSTM dropout probability
        fusion_mode: Feature fusion mode ('concat' or 'gated')
        fusion_out_dim: Output dimension after fusion (None for auto)
    """
    ...
    def __init__(
        ...
        self.emb_event = nn.Embedding(
            vocab_size, emb_event_dim, padding_idx=pad_idx
        )
        self.emb_cats = nn.ModuleDict({
            c: nn.Embedding(
                cat_vocab_sizes[c], emb_cat_dim, padding_idx=pad_idx
            )
        ...
        self.num_proj = (
            nn.Linear(self.num_in_dim, num_proj_dim)
        ...
        self.base_dim = (
            emb_event_dim
            + len(self.cat_cols) * emb_cat_dim
            + (num_proj_dim if self.num_proj else 0)
        )
        ...
        self.fusion = MultiModalFusion(
            ...
            out_dim=fusion_out_dim or self.base_dim,   # keep same width 
    ...
    def forward(
        ...
        if self.num_proj:
            num_stack = torch.stack(
                [num_feats[c] for c in self.num_cols], dim=-1
            )  # [B,T,K]
            num_stack = torch.nan_to_num(
                num_stack, nan=0.0, posinf=1e6, neginf=-1e6
            )

"""Data loading and feature engineering for task-mining event logs.

This module provides EventLogDataBuilder for loading raw CSV event logs
and extracting meaningful features for machine learning models.
...
Usage:
    ...
    df = builder.load_and_prepare("data/events.csv")

The resulting DataFrame includes enriched features:
    - action_type: Extracted from event_name (Write, Read, Action*, etc.)
    - target_app: Application name from event_name
    - file_extension: File extension if present
    - file_path_depth: Depth of file paths
    - is_sharepoint: Boolean indicating SharePoint usage
    - event_name_length: Length of event name string
    - first_number: First numeric value found in event name
    - elapsed_time: Time since previous event in session
...
class EventFeatureExtractor:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out["is_sharepoint"] = out[col].astype(str).str.contains(
            "sharepoint", case=False, regex=False
        )
...
class TimeFeatureEngineer:
    ...
    def __init__(
        self, timestamp_col: str = "timestamp", session_col: str = "session_id"
    ):
    ...
    def _ensure_datetime(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out[self.timestamp_col] = pd.to_datetime(
            out[self.timestamp_col], format="mixed", errors="coerce"
        )
        return out

    def add_elapsed_time(self, df: pd.DataFrame) -> pd.DataFrame:
        """Add elapsed time feature within each session.
        
        Args:
            df: DataFrame with timestamp and session columns
            
        Returns:
            DataFrame with elapsed_time column added
        """
        out = self._ensure_datetime(df)
        out = out.sort_values(by=[self.session_col, self.timestamp_col])
        deltas = (
            out.groupby(self.session_col)[self.timestamp_col]
            .diff()
            .dt.total_seconds()
        )
    ...
    def add_session_duration(self, df: pd.DataFrame) -> pd.DataFrame:
        """Add session duration feature for each session.
        
        Args:
            df: DataFrame with timestamp and session columns
            
        Returns:
            DataFrame with session_duration column added
        """
        out = self._ensure_datetime(df)
        grp = out.groupby(self.session_col)[self.timestamp_col]
        duration = grp.transform(
            lambda x: (
                (x.max() - x.min()).total_seconds() if x.notna().all() else np.nan
            )
        )
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        """Apply all time-based feature engineering.

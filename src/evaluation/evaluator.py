"""Evaluation utilities for sequence tagging models.

This module provides comprehensive evaluation functionality including:
- Standard evaluation metrics (accuracy, F1-score)  
- Selective evaluation with confidence thresholding
- Detailed misclassification analysis
- Support for both legacy and multimodal batch formats

Key functions:
    - evaluate_plain: Standard evaluation on all predictions
    - evaluate_selective: Evaluation with confidence-based abstention
    - _misclassification_summary: Detailed confusion analysis
"""
...
def _unpack_batch(batch):
    """Unpack batch supporting both legacy 5-tuple and new 7-tuple formats.
    
    Supports backward compatibility with batches that don't include
    text and image features.
    
    Args:
        batch: Either 5-tuple (tokens, labels, mask, cat_feats, num_feats)
               or 7-tuple (..., txt_feats, img_feats)
    
    Returns:
        7-tuple: tokens, labels, mask, cat_feats, num_feats, txt_feats, img_feats
    """
    if len(batch) == 5:
...
def _misclassification_summary(
    ...
    for i in range(num_classes):
        ...
        if total > 0 and cnt > 0:
            lines.append(
                f"  {label_names[i]} most often confused with "
                f"{label_names[j]} : {cnt} ({cnt/total:.2%})"
            )
        elif total > 0:
            lines.append(
                f"  {label_names[i]}: no confusions (perfect on observed examples)."
            )
        else:
